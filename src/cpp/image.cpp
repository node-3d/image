...
JS_IMPLEMENT_GETTER(Image, width) {
	THIS_CHECK;
	RET_NUM(_bitmap ? FreeImage_GetWidth(_bitmap) : 0);
}

JS_IMPLEMENT_GETTER(Image, height) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Image, _load) {
	THIS_CHECK;
	REQ_BUF_ARG(0, file);
...
JS_IMPLEMENT_METHOD(Image, _unload) {
	THIS_CHECK;
	if (_bitmap) {
		FreeImage_Unload(_bitmap);
		_bitmap = nullptr;
...
JS_IMPLEMENT_METHOD(Image, save) {
	THIS_CHECK;
	REQ_STR_ARG(0, dest);
...
JS_IMPLEMENT_METHOD(Image, drawImage) {
	THIS_CHECK;
	REQ_OBJ_ARG(0, _src);
	...
	FIBITMAP *result =
	    FreeImage_RescaleRect(src->_bitmap, dWidth, dHeight, sx, sy, sx + sWidth, sy + sHeight);
	if (!result) {
...
JS_IMPLEMENT_METHOD(Image, destroy) {
	THIS_CHECK;
	emit(info, "destroy");
	_destroy();
	RET_UNDEFINED;
...
JS_IMPLEMENT_GETTER(Image, isDestroyed) {
	NAPI_ENV;
